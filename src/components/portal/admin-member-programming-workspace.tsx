...
type AdminMemberProgrammingWorkspaceProps = {
...
export function AdminMemberProgrammingWorkspace({
  assignments,
  documents,
  memberId,
  memberName,
}: AdminMemberProgrammingWorkspaceProps) {
    ...
    return (
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <AdminDocumentForm
          ...
          heading={`Upload a file for ${memberName}`}
        />
      </div>
    );
  }

  const sectionKeys = getPopulatedPlanSectionKeys(selectedAssignment.plan.sections);
  ...
  const activeSectionValue = selectedAssignment.plan.sections[activeSectionKey]?.trim();
      ...
      <div className="rounded-[1.35rem] border border-[var(--line)] bg-white/78 px-5 py-5">
        ...
        <div className="mt-5 rounded-[1.2rem] border border-[rgba(141,107,61,0.16)] bg-[rgba(141,107,61,0.07)] px-5 py-5">
          ...
          <div className="mt-4 whitespace-pre-line text-sm leading-7 text-[var(--ink)]">
            {activeSectionValue || "No details saved in this section yet."}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_0.95fr]">
        <div className="surface-panel p-6 sm:p-8">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
            Files uploaded while this section is active stay grouped here.
          </p>
          ...
          <div className="mt-6 grid grid-cols-1 gap-4">
            {sectionDocuments.length ? (
              sectionDocuments.map((document) => (
                ...
                >
                  <h4 className="text-lg font-semibold text-[var(--ink)]">{document.title}</h4>
        ...
        <AdminDocumentForm
          ...
          memberName={memberName}
          heading={`Upload a ${PLAN_SECTION_LABELS[activeSectionKey].toLowerCase()} file`}
          hideSectionPicker
        />
      </div>
    </div>
